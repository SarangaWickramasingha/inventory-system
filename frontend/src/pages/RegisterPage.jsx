...
import { Box, Lock, Mail, User, ArrowRight, CheckCircle2, AlertCircle, Shield, Eye, EyeOff, X } from 'lucide-react';
...
export const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result && result.success) {
      setSuccess('Registration submitted successfully! Your staff account is currently pending Admin approval. You will be able to sign in once an Administrator approves your request.');
      setTimeout(() => {
        ...
        <div className="w-full max-w-md my-auto bg-white/10 backdrop-blur-2xl py-8 px-6 shadow-2xl rounded-3xl border border-white/20 sm:px-10 text-white relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-xs text-slate-300 font-medium">
              Register a new Staff account for inventory operations
            </p>
          </div>

          {/* Account Type Banner */}
          <div className="mb-6 p-3 bg-blue-500/20 border border-blue-400/30 rounded-xl flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white shrink-0 font-bold border border-blue-400/30">
              <Shield className="w-4 h-4" />
            </div>
            <div>
              <div className="text-[10px] font-extrabold text-blue-300 uppercase tracking-wider">Account Role</div>
              <div className="text-xs font-bold text-white">Staff Member (Inventory & Stock Operations)</div>
            </div>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 active:from-blue-700 active:to-indigo-700 text-white font-extrabold text-sm rounded-xl shadow-lg shadow-blue-500/25 transition-all flex items-center justify-center gap-2 group disabled:opacity-70 mt-6 border border-blue-400/30"
            >
              {isLoading ? (
